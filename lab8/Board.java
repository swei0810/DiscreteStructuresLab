...
public class Board {
    ...
    /**
     * Return next player assuming X goes first.
     * @return mark of the player whose turn is next
     */
    public Mark nextPlayer() {
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                if (board[i][j].equals(Mark.X)) {
                    xCount++;
                } else if (board[i][j].equals(Mark.O)) {
                    oCount++;
                }
            }
        }
    ...
    /**
     * Removes ark at given (row,column)
     * @param row row to place mark (0 <= row < n)
     * @param col column to place mark (0 <= col < n)
     * @throws RuntimeException for invalid row/col or if placing mark in a blank space
     */
    public void removeMark(int row, int col) {
        checkRowAndCol(row, col);
        if (board[row][col].equals(Mark.BLANK)) {
            throw new RuntimeException("This space is currently empty... nothing to remove!");
        }
